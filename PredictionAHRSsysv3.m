function [x P x2 P2]= PredictionAHRSsysv3(u,x,P,x2 ,P2)
global PAR;
global GVAR;
...
delta_t = PAR.delta_t;
 ...
 lambda_w = PAR.lambda_w; 
           ...
           u.w_bn_b = u.w_bn_b +  PAR.ExtraBias; % Add extra bias
          ...
          % gyro bias
           ...
           x(5:7) = -u.w_bn_b*GiroScale  + x(8:10);
              ...
              % Mechanization Equation  (D.36)**************************
               ...
               % w_u = - (u.w_bn_b*GiroScale - x(8:10)    + PAR.ExtraBias);  
               w_u = x(5:7);
               ...
               % end;
                ...
                x(1:4,1) = qp;   %% predicted quaternion
              
             
             %**********************************************************
             %  Jacobians
             xb = x(8:10);
             [JFx, JFu]  =  JacFx7(x,w_u,delta_t,xb,lambda_w,lambda_g);             
...
if PAR.IndirectMethodActive == 1
             ...
             w_bn_b =  -(u.w_bn_b'*GiroScale -Gbias'  );  
             % w_bn_b =  -(u.w_bn_b'*GiroScale   + PAR.ExtraBias');   
             ...
             gyro_nrm = norm(w_bn_b);
              ...
              % Mechanization Equation  (D.36)**************************
               ...
               % end;
                ...
                Rn2b    =(quat2R(q)+quat2R(x2(1:4,1)))/2; 
                Rb2n    = Rn2b';
                % State Space Error Model,  combination of (10.9), (10.11)
                % and (10.57)
                Fg  = -PAR.lambda_g*eye(3,3);   %(10.9)
                Fa  = -PAR.lambda_a*eye(3,3);   % (10.12)
                ...
                Q = [I*sigma_in^2 Z          Z           Z
                     ...
                     Z            Z     I*(sigma_nu_g*1)^2   Z
                     Z            Z          Z       I*PAR.sigma_xa^2*0]; 
                ...
                Phi = phi*GVAR.Phi;          % accumulate phi,  (CH7)
                Qd  = phi*(q+GVAR.Qd)*phi';  % accumulate Qd,   (CH7)
