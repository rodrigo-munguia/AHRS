function [x P x2 P2]= InitAHRSsysv2(u,x,P,x2,P2)
global PAR;
global GVAR;
...
gyro_nrm = norm(w_bn_b);
           ...
           % int_w = int_w + -(w_bn_b*GiroScale) ; % integrating giro measurements (without bias) in repose for obtaining bias
...
if PAR.IndirectMethodActive == 1
     ...
     x2(1:4,1) =  qini; %initial quaternion
     if PAR.IncludeInitialGyroBiasEstimation == 1;
        x2(5:7,1) = xg;  % initial giro bias in radians  x_g
     else
        x2(5:7,1) = [0; 0; 0];  % initial giro bias in radians  x_g
     end;
     x2(8:10,1)   = [0;0;0]; %initial accelerometers bias x_a
     
     P2 = Pini(sigma_nu_a,sigma_nu_g,int_T); % estimate initial covariance matrix   
else
